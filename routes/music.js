...
router.get('/getSong', async function (req, res, next) {
    results = await pool.query("select * from canciones");
...
router.post('/getSong', async function (req, res) {
    ...
    await pool.query("select * from canciones where cancion  ilike $1" , [ '%' + cancion + '%' ],
        function (err, result) {
            if (err) throw err;
            if (result) {
                res.setHeader("content-type", "application/json")
...
router.post('/getAlbum', async function (req, res) {
    ...
    await pool.query("select * from albumes where nombre ilike $1" , [ '%' + album + '%' ],
        function (err, result) {
            if (err) throw err;
            if (result) {
                res.setHeader("content-type", "application/json")
